...
impl<'ctx> SomeValue<'ctx> {
    pub fn as_local_pointer(&self) -> Result<&PointerValue<'ctx>, CodeGenError> {
        match self {
            SomeValue::Local(ptr) => Ok(*ptr),
            _ => Err(CodeGenError::cannot_convert_to_local_pointer(None)),
        }
    }

    pub fn as_global(&self) -> Result<&GlobalValue<'ctx>, CodeGenError> {
        match self {
            SomeValue::Global(val) => Ok(*val),
            _ => Err(CodeGenError::cannot_convert_to_global_value(None)),
        }
    }
...
impl<'a> CompiledCase<'a> {
    pub fn new(cond: &'a ConstExpr, block: BasicBlock<'a>, code: Option<AnyValueEnum<'a>>, insert_block: Option<BasicBlock<'a>>) -> CompiledCase<'a> {
        CompiledCase::Case {
            cond,
            block,
            code,
    ...
    pub fn new_default(block: BasicBlock<'a>, code: Option<AnyValueEnum<'a>>, insert_block: Option<BasicBlock<'a>>) -> CompiledCase<'a> {
...
impl<'ctx> Env<'ctx> {
    ...
    pub fn insert_case(&mut self, cond: &'ctx ConstExpr, block: BasicBlock<'ctx>, code: Option<AnyValueEnum<'ctx>>, insert_block: Option<BasicBlock<'ctx>>) {
        self.local_cases.last_mut().unwrap().last_mut().unwrap().push(CompiledCase::new(cond, block, code, insert_block));
    }

    pub fn insert_default(&mut self, block: BasicBlock<'ctx>, code: Option<AnyValueEnum<'ctx>>, insert_block: Option<BasicBlock<'ctx>>) {
        self.local_cases.last_mut().unwrap().last_mut().unwrap().push(CompiledCase::new_default(block, code, insert_block));
    }
    ...
    pub fn insert_typedef(&mut self, key: &str, typ: &Type, ctx: &'ctx Context) -> Result<(), Box<dyn Error>> {
        ...
        let type_or_union = match typ {
            Type::Symbol(name) => {
                let t = self.get_type(name).ok_or(CodeGenError::no_such_a_type(None, name))?;
                t.clone()
            },
            Type::Struct { name, fields } => {
                if let Some(id) = name {
                    ...
                    let (t, _index_map) = CodeGen::struct_from_struct_definition(name, fields, ctx)?;
            ...
            Type::Union { name, fields } => {
                if let Some(id) = name {
                    ...
                    let (type_list, index_map, max_size, max_size_type) = CodeGen::union_from_struct_definition(name, fields, ctx)?;
                    TypeOrUnion::Union {
                        type_list: type_list,
                        index_map: index_map,
                        max_size: max_size,
            ...
            _ => TypeOrUnion::Type(TypeUtil::to_llvm_any_type(typ, ctx)?),
        };
                ...
                if let Some(id) = name {
                    if let Some(type_or_union) = self.get_type(id) {
                        match type_or_union {
                            ...
                            TypeOrUnion::Union { max_size_type, .. } => {
                                let t = max_size_type.ok_or(CodeGenError::union_has_no_field(None, None))?;
                                if let Ok(basic_type) = BasicTypeEnum::try_from(t) {
                                    Ok(basic_type)
                                }else{
                        ...
                        let (any_type, _index_map) = CodeGen::struct_from_struct_definition(&None, fields, ctx)?;
            ...
            Type::Union { name, fields } => {
                if let Some(id) = name {
                    if let Some(type_or_union) = self.get_type(id) {
                        ...
                        let (_type_list, _index_map, _max_size, max_size_type_opt) = CodeGen::union_from_struct_definition(&None, fields, ctx)?;
                        let max_size_type = max_size_type_opt.ok_or(CodeGenError::union_has_no_field(None, None))?;
                        Ok(max_size_type)
                    }
                }else{
                    let (_type_list, _index_map, _max_size, max_size_type_opt) = CodeGen::union_from_struct_definition(&None, fields, ctx)?;
    ...
    pub fn insert_struct(&mut self, key: &str, struct_type: &StructType<'ctx>, index_map: HashMap<String, usize>) -> Result<(), CodeGenError> {
        if let Some(_any_type_enum) = self.types.get(key) {
            return Err(CodeGenError::already_type_defined_in_struct(None, key));
        }
    ...
    {
        if let Some(_any_type_enum) = self.types.get(key) {
            return Err(CodeGenError::already_type_defined_in_union(None, key));
        }
    ...
    pub fn insert_enum(&mut self, key: &str, enum_type: &IntType<'ctx>, enumerator_list: Vec<(String, IntValue<'ctx>)>, index_map: HashMap<String, usize>) -> Result<(), CodeGenError> {
        if let Some(_any_type_enum) = self.types.get(key) {
            return Err(CodeGenError::already_type_defined_in_struct(None, key));
        }
        ...
        match ast {
            ExprAST::Symbol(name, _pos) => {
                if let Some((typ, _pointer)) = self.get_ptr(name) {
                    ...
                    Err(CodeGenError::condition_is_not_number(None, ast))
                }
            },
            _ => Ok(ast.is_signed()?),
        }
