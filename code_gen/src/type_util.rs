...
impl TypeUtil {
    ...
    pub fn to_basic_type_enum<'a>(typ: &Type, ctx: &'a Context, env: &mut Env<'a>, pos: &Position) -> Result<BasicTypeEnum<'a>, Box<dyn Error>> {
        match typ {
            ...
            Type::Number(NumberType::Double) => Ok(BasicTypeEnum::FloatType(ctx.f64_type())),
            Type::Number(NumberType::Float)  => Ok(BasicTypeEnum::FloatType(ctx.f32_type())),
    ...
    pub fn to_llvm_any_type<'a>(typ: &Type, ctx: &'a Context, env: &mut Env<'a>, pos: &Position) -> Result<AnyTypeEnum<'a>, Box<dyn Error>> {
        match typ {
            ...
            Type::Number(NumberType::Float)  => Ok(AnyTypeEnum::FloatType(ctx.f32_type())),
            Type::Number(NumberType::Double) => Ok(AnyTypeEnum::FloatType(ctx.f64_type())),
    ...
    pub fn make_llvm_ptr_type<'a>(typ: &Type, ctx: &'a Context, env: &mut Env<'a>, pos: &Position) -> Result<PointerType<'a>, Box<dyn Error>> {
        match typ {
            ...
            Type::Number(NumberType::Float)  => Ok(ctx.f32_type().ptr_type(AddressSpace::default())),
            Type::Number(NumberType::Double) => Ok(ctx.f64_type().ptr_type(AddressSpace::default())),
