...
println!("make_global. TYPE: {:?}", typ);
    ...
    fn make_global_def_array(&self, ds: DeclarationSpecifier, declarations: Vec<Declaration>, opt_size_list: &Vec<Option<ConstExpr>>, defs: &mut Defines, pos: &Position) -> Result<AST, ParserError> {
        let typ = ds.get_type();
...
println!("goto make_global 2. declaration: {:?}", declaration);
                            Ok(Some(self.make_global_def_var(ds.clone(), vec![declaration], defs, pos)?))
                        },
                        _ => {
                            println!("Syntax Error. {}:{}:{}", file!(), line!(), column!());
...
println!("parse_declarator. opt_ptr: {:?}, direct: {:?}", opt_pointer, direct);
...
println!("sq: {:?}, type_or...: {:?}, opt_abs_decl: {:?}", sq, type_or_variadic, opt_abstract_decl);
...
println!("parse_constant");
        if let Some((tok, pos)) = iter.peek() {
println!("in parse_constant. tok: {:?}", tok);
            match &*tok {
        ...
        if let Some((tok, pos)) = iter.peek() {
            match *tok {
                Token::Comma => {
                    ...
                    // let (tok2, _pos2) = iter.next().ok_or(ParserError::illegal_end_of_input(pos.clone()))?;
                    let (tok2, pos2) = iter.next().unwrap();
                    if tok2.is_eof() { return Err(ParserError::illegal_end_of_input(pos2.clone())); }
...
println!("sub");
    ...
    fn parse_initializer(&self, iter: &mut Peekable<Iter<(Token, Position)>>, defs: &mut Defines, labels: &mut Option<&mut Vec<String>>) -> Result<ExprAST, ParserError> {
        ...
        if *tok == Token::BraceLeft {
            ...
            init_expr = self.parse_assignment_expression(iter, defs, labels)?.ok_or(ParserError::need_expr(pos.clone()))?;
        }
        Ok(init_expr)
    }
...
println!("( in parse_for");
    ...
    fn parse_expected_token(&self, iter: &mut Peekable<Iter<(Token, Position)>>, expected: Token) -> Result<Position, ParserError> {
        ...
        // }
        let (tok, pos) = iter.next().unwrap();
println!("parse_expected_token: {:?}", tok);
        if *tok == expected {
            Ok(pos.clone())
        }else{
            Err(ParserError::without_expected_token(pos.clone(), expected.clone(), tok.clone()))
