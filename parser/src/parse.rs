                ...
                Token::Return => {
                    ...
                    if *t == Token::SemiColon {
                        ...
                        if let Some(ast) = self.parse_expression(iter, defs, labels)? {
                            ...
                            Err(ParserError::illegal_end_of_input(Some(pos.clone())))
                        }
                    }
                },
                // labeled-statement
                ...
                AST::Expr(Box::new(
                    ExprAST::Assign(
                        Box::new(ExprAST::MemberAccess(
                            Box::new(ExprAST::Symbol(String::from("date"))),
                            String::from("year")
                        )),
                        Box::new(ExprAST::Int(2022))
                )))
            );
            assert_eq!(
                list[2],
                ...
                AST::Expr(Box::new(
                    ExprAST::Assign(
                        Box::new(ExprAST::PointerAccess(
                            Box::new(ExprAST::Symbol(String::from("pointer"))),
                            String::from("year")
                        )),
                        Box::new(ExprAST::Int(2023))
                )))
            );
            assert_eq!(
                list[6],
