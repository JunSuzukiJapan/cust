...
// use super::{CompileError, StructDeclaration, SpecifierQualifier, Defines, ConstExpr};
...
impl NumberType {
    // pub fn to_basic_type_enum<'ctx>(&self, ctx: &'ctx Context) -> Result<BasicTypeEnum<'ctx>, CompileError> {
    //     match self {
    ...
    //         _ => Err(CompileError::cannot_convert_to_basic_type_enum(None)),
    //     }
    // }

    pub fn to_unsigned(&self) -> Result<NumberType, CompileError> {
        match self {
            NumberType::Char => {
            ...
            NumberType::Float | NumberType::Double | NumberType::_Bool => {
                Err(CompileError::cannot_to_be_unsigned(None, &Type::Number(self.clone())))
            },
            _ => Ok(self.clone())
        }
    }
...
impl StructDefinition {
    pub fn try_new(struct_name: Option<String>, fields: Option<Vec<StructDeclaration>>) -> Result<StructDefinition, CompileError> {
    ...
    pub fn make_fields_from_vec(fields: Vec<StructDeclaration>) -> Result<(Vec<StructField>, HashMap<String, usize>), CompileError> {
            ...
            for d in list {
                ...
                if let Some(size) = d.get_bit_size() {
                    ...
                    if let Some(t) = typ {
                        ...
                        return Err(CompileError::no_type_for_struct_field(None));
                    }
                }
    ...
    pub fn try_size_of(&self, ctx: &Context, _defs: &Defines) -> Result<u64, CompileError> {
        let (struct_type, _map) = CodeGen::struct_from_struct_definition(&None, self, ctx)?;
        let int_value = struct_type.size_of().ok_or(CompileError::cannot_size_of_struct(None))?;
        let size = int_value.get_zero_extended_constant().ok_or(CompileError::cannot_size_of_struct(None))?;
        Ok(size)
    }
...
impl Type {
    ...
    pub fn get_pointed_type(&self) -> Result<&Type, CompileError> {
        if let Type::Pointer(_pointer, to_type) = self {
            Ok(&**to_type)
        }else{
            Err(CompileError::not_pointer(None, self))
        }
    }

    pub fn get_element_type(&self) -> Result<&Type, CompileError> {
        if let Type::Array { typ, .. } = self {
            Ok(&**typ)
        }else{
            Err(CompileError::not_array(None, self))
        }
    }

    pub fn get_fn_ret_type(&self) -> Result<&Type, CompileError> {
        if let Type::Function { ret_type, .. } = self {
            Ok(&ret_type)
        }else{
            Err(CompileError::not_function(None, self))
        }
    }
    ...
    pub fn get_number_type(&self) -> Result<&NumberType, CompileError> {
        match self {
            Type::Number(nt) => Ok(nt),
            _ => Err(CompileError::not_number_type(None, self)),
        }
    }

    pub fn to_unsigned(&self) -> Result<Type, CompileError> {
        match self {
            Type::Number(nt) => {
            ...
            _ => Err(CompileError::not_number_type_to_be_unsigned(None, self)),
        }
    }

    pub fn to_basic_type_enum<'a>(&self, ctx: &'a Context) -> Result<BasicTypeEnum<'a>, CompileError> {
        match self {
            ...
            Type::Pointer(_p, to_type) => {
                ...
                if typ.is_int_type() {
                    ...
                    Err(CompileError::illegal_type_for_pointer(None, &to_type))
                }
            ...
            Type::Union { name, fields } => {
                ...
                if let Some(typ) = max_size_type {
                    ...
                    if let Some(id) = name {
                        Err(CompileError::union_has_no_field(None, Some(id)))
                    }else{
                        Err(CompileError::union_has_no_field(None, None))
                    }
                }
            },
            ...
            _ => {
                Err(CompileError::no_such_a_type(None, &self.to_string()))
            },
        }
    }


    pub fn to_llvm_type<'a>(&self, ctx: &'a Context) -> Result<BasicMetadataTypeEnum<'a>, CompileError> {
        Ok(self.to_basic_type_enum(ctx)?.into())
    }

    pub fn to_llvm_any_type<'a>(&self, ctx: &'a Context) -> Result<AnyTypeEnum<'a>, CompileError> {
        match self {
